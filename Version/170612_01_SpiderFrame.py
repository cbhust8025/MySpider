...
class Spider(wx.Frame):
    ...
    def __init__(self):
        ...
        # 初始化一些窗口内的属性值
        self.namelink = {}  # 保存已经爬取的{小说名：[分类，链接]} 键值对，用于自动补全的小说索引链接
        ...
        self.combocontrol = wx.combo.ComboCtrl(
            parent=self.Panel1, style=wx.CB_SORT)  # 初始化只读格式的组合框
        ...
        self.combocontrol.SetButtonPosition(
            width=combocontrolButtonWidth)  # 配置组合框的下拉按钮的宽度
    ...
    def GetLinksForRecommand(self, soup):  # 获取起点主页的所有小说链接
        LinksRes = soup.find_all('a', {  # 小说链接所在Tag标签名为a
            ...
            # 小说链接所在Tag的“data-eid”形式为qd_A110这种样式，使用正则表达式进行匹配
        ...
        for Link in LinksRes:
            # 离小说最近的前置Tag名为'h3'的Tag即为当前小说所在的分类，“NO.1”这个标签除外
    ...
    def OnBtnBackButton(self, event):  # 自定义按键响应
        ...
        elif event.GetKeyCode() == 9 or event.GetKeyCode() == 317:  # 如果是Tab键 或者方向键中的down键
                ...
                if(self.mcListPopup.GetSelectedItemCount() == 0
                   or self.mcListPopup.GetSelection() == self.mcListPopup.GetItemCount() - 1):
                    ...
                    while(self.mcListPopup.ScrollPages(-1)):
                        # 如果已经选中了最后一个item，然后按下Tab键，则一直向上翻页直到最上面
                        pass
                else:
                    ...
                    self.mcListPopup.Select(
                        self.mcListPopup.GetSelection(), False)  # 将当前选中的取消选中
        ...
        elif event.GetKeyCode() == 27:  # 如果当前按下的键为ESC键，则关闭下拉框
            if self.combocontrol.IsPopupShown():  # 如果已经打开下拉框
                self.combocontrol.HidePopup()  # 隐藏下拉框
                self.combotext.ChangeValue("")  # 清空当前文本域内的所有内容,并且不产生text chhange event
        ...
        if(self.combotext.GetValue() == ""):  # 如果当前组合框文本为空，则下拉推荐列表显示
            ...
            for name in names:
                self.mcListPopup.Append(
                    [name, self.namelink[name][-2], self.namelink[name][-1]])  # 利用Append方法添加一行文本
    ...
    def BarTextValueSet(self, value):
        ...
        f = wx.Font(15, wx.ROMAN, wx.NORMAL, wx.BOLD,
                    False)  # 设置状态栏文本字体：15号 罗曼字体， 不倾斜、加粗
